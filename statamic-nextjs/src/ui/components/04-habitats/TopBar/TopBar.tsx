...
export const TopBar = ({ nav, asPath }: Props) => {
          ...
          {nav.tree.map(({ page }, index) => (
            <Link
              className={classNames(
                ...
                {
                  'z-10 border-transparent hover:bg-white': currentPath !== page.url,
                },
                {
                  'bg-white border-pages-500 font-bold': currentPath === page.url,
                }
              )}
              href={page.url}
              key={index}
